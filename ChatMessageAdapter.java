            ...
            if (message.getStatus() == ChatMessage.STATUS_INDEXING_PROGRESS) {
                ...
                if (message.getProposedFileChanges() != null && !message.getProposedFileChanges().isEmpty()) {
                    ...
                    for (ChatMessage.FileActionDetail detail : message.getProposedFileChanges()) {
                        ...
                        switch (detail.type) {
                            case "createFile":
                                labelText = "New";
                                labelBgColorResId = R.color.color_status_new;
                                strokeColorResId = R.color.color_diff_added_stroke;
                                fileChangeCardView.setCardBackgroundColor(ContextCompat.getColor(context, R.color.color_diff_added_bg));
                                fileChangeCardView.setStrokeColor(ContextCompat.getColor(context, strokeColorResId));
                                break;
                            case "deleteFile":
                                labelText = "Deleted";
                                labelBgColorResId = R.color.color_status_deleted;
                                strokeColorResId = R.color.color_diff_deleted_stroke;
                                fileChangeCardView.setCardBackgroundColor(ContextCompat.getColor(context, R.color.color_diff_deleted_bg));
                                fileChangeCardView.setStrokeColor(ContextCompat.getColor(context, strokeColorResId));
                                break;
                            case "modifyLines":
                            case "updateFile":
                                labelText = "Updated";
                                labelBgColorResId = R.color.color_status_updated;
                                strokeColorResId = R.color.color_diff_updated_stroke;
                                fileChangeCardView.setCardBackgroundColor(ContextCompat.getColor(context, R.color.color_diff_updated_bg));
                                fileChangeCardView.setStrokeColor(ContextCompat.getColor(context, strokeColorResId));
                                break;
                            case "renameFile":
                                labelText = "Renamed";
                                labelBgColorResId = R.color.color_status_renamed;
                                strokeColorResId = R.color.color_diff_renamed_stroke;
                                fileChangeCardView.setCardBackgroundColor(ContextCompat.getColor(context, R.color.color_diff_renamed_bg));
                                fileChangeCardView.setStrokeColor(ContextCompat.getColor(context, strokeColorResId));
                                break;
                            default:
                ...
                if (message.getActionSummaries() != null && !message.getActionSummaries().isEmpty()) {
                    ...
                    for (String summary : message.getActionSummaries()) {
                        ...
                        layoutActionSummaries.addView(tv);
                    }
                } else {
                    layoutActionSummaries.setVisibility(View.GONE);
                }
